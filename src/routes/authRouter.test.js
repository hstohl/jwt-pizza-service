...
test('register', async () => {
    ...
    // expect(lregRes.body.token).toMatch(/^[a-zA-Z0-9\-_]*\.[a-zA-Z0-9\-_]*\.[a-zA-Z0-9\-_]*$/);
  ...
  test('register without password', async () => {
    ...
    // const { password, ...myUser } = { ...user, roles: [{ role: 'diner' }] };
    // expect(regRes.body.user).toMatchObject(myUser);
  });
